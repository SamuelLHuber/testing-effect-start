...
import { ZerionPortfolioResponseSchema } from "../../../lib/zerion-schemas"
...
export const GET = Effect
  .gen(function*() {
    ...
    const zerionPath = `/v1/wallets/${address}/portfolio${queryString ? `?${queryString}` : ""}`

    const rawData = yield* zerionFetch(zerionPath)

    // Validate response with schema
    const validatedData = yield* Schema.decodeUnknown(
      ZerionPortfolioResponseSchema
    )(rawData)
